# sd-nodes-fj
Some custom nodes for Substance Designer. Here is a list in approximate order of how useful or good they are:
...
### Greyscale to Mask
Just like the Histogram Select node, but with a colour picker, similar to the stock Colour to Mask node.

### Scale & Bias
Similar to Unreal Engine's ConstantBiasScale shader node. Primarily made to remap \[0,1\] data to \[-1,1\]. Useful when directly authoring normal maps, and can be fun for directional warps.
